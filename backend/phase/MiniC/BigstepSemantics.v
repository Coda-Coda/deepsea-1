...
(*     to_plus. *)
(*     econstructor; split. apply star_one. *)
(*     econstructor; eauto. constructor; auto; lia. *)
(*  *)
(* (* call *) *)
(*   - intros. *)
(*     to_plus. *)
...
(*       * lia. *)
(*       * exact I. *)
(*     + replace (x + g + gas_saveretval' 1 - g) with (x + gas_saveretval' 1) by lia. *)
...
(*     replace (x + g1 + g2 - g1) with (x+g2) by lia. eexact A2. *)
(*     replace (x + g1 + g2 - (g1 + g2)) with (x + g2 - g2) by lia. *)
(*     exact B2. *)
(*  *)
(* (* sequence 2 *) *)
(*   - to_plus.  *)
...
(*     congruence. *)
(*     * apply star_one. *)
(*       replace (x + g1 - g1) with x by lia. *)
(*       apply step_break_seq. *)
(*     * apply star_refl. *)
(*   + unfold S2; inv B1; congruence || econstructor; eauto; lia. *)
(*  *)
(* (* ifthenelse *) *)
(*   - to_plus. *)
...
(*   (*       *)
(*   -  econstructor; split. apply star_refl. *)
(*      apply osm_return_none; simpl; auto; lia. *)
(*    *) *)
(*  *)
(*  *)
(* (* return some *) *)
(*   -econstructor; split. apply star_refl. *)
(*    apply osm_return_some; simpl; auto; lia. *)
(*  *)
(*  *)
(* (* break *) *)
(*   - econstructor; split. apply star_refl. *)
(*     apply osm_break; simpl; auto; lia. *)
(*  *)
(* (* loop stop *) *)
(*   - to_plus. rewrite !plus_assoc. *)
...
(*       inv B1. *)
(*       eapply star_left. *)
(*       replace (x + g1 + g2 - g1) with (x + g2) by lia. *)
(*       rewrite A2_eq2. *)
...
(*  *)
(*       Focus 2. *)
(*       rewrite <- gas_return_done_split; f_equal; lia. *)
